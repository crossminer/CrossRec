...
//						System.out.println(vector1[i] + ":" + vector2[i]);
	...
	public Map<String, Integer> getProjectAlternativesWithSimilarAPIs(String inputProject) {
		...
		if (trainingStartPos1 < trainingEndPos1)
			trainingProjects = reader.readProjectList(this.srcDir + "projects.txt", trainingStartPos1, trainingEndPos1);

		if (trainingStartPos2 < trainingEndPos2) {
			Map<Integer, String> tempoProjects = reader.readProjectList(this.srcDir + "projects.txt", trainingStartPos2,
					trainingEndPos2);
			trainingProjects.putAll(tempoProjects);
		}
